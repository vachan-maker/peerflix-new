...
// Configured for single connection per peer (reduces connection spam)
const client = new WebTorrent({
    maxConns: 10,  // Maximum total connections across all torrents (reduced from default 55)
...
// Track unique peer IPs to avoid logging duplicate connections
const connectedPeerIPs = new Set();
        ...
        try {
            ...
            torrent.on('wire', (wire, addr) => {
                ...
                if (!connectedPeerIPs.has(ip)) {
                    ...
                    console.log(`🔗 New peer connected to ${torrent.name}: ${ip} (Total unique peers: ${connectedPeerIPs.size})`);
                }
            });
